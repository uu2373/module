...
class ModelClients{
  ...
  const SQL_SEARCHCLIENT   = 'SELECT id,name FROM clients where upper(name) like upper(:NAME)';


  function GetClient($name = '') {
    $data = array();
    //$name = $this->ToWIN($name);
    try {
      $res = $this->db->query(self::SQL_SEARCH)->execute(array('NAME'=>"%$name%"));
      while ($res->next()) {
        $data[] = $res->current();
      }
    } catch (Exception $e) {
